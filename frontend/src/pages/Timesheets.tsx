...
import { AttendanceStatus, STATUS_BADGE, STATUS_COLOR } from "@/lib/types";
...
export default function Timesheets() {
  const { user } = useAuth();
  const all = ATTENDANCE[user!.id] || [];

  const [range, setRange] = useState(defaultRange());
  const [exportOpen, setExportOpen] = useState(false);

  const filtered = useMemo(
    () => filterRecords(all, range).sort((a, b) => b.date.localeCompare(a.date)),
    [all, range]
  );
